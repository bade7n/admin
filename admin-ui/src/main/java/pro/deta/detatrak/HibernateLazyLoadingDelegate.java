...
public class HibernateLazyLoadingDelegate implements LazyLoadingDelegate {
	...
	private <E> Object lazilyLoadPropertyValue(E entity, String prop) {
		...
		q.where(cb.equal(root.get(getIdProperty(entity.getClass())), cb.literal(tryGetEntityId(entity))));
		return entityProvider.getEntityManager().createQuery(q).getResultList();
	}

	public <E> String getIdProperty(Class<E> entityClass) {
		String id = "id";
		Metamodel metamodel = entityProvider.getEntityManager().getMetamodel();
		EntityType<E> entityType = (EntityType<E>) metamodel.entity(entityClass);
		if(entityType.getIdType() != null) {
			Class<?> idClass = entityType.getIdType().getJavaType();
			if(idClass != null) {
				SingularAttribute<? super E, ?> idAttr = entityType.getId(idClass);
				if(idAttr != null)
					id = idAttr.getName();
			}
		}
